...
func (*UserCreditRecord) scanValues(columns []string) ([]interface{}, error) {
	values := make([]interface{}, len(columns))
	for i := range columns {
		switch columns[i] {
		case usercreditrecord.FieldID, usercreditrecord.FieldCreatedAt, usercreditrecord.FieldUpdatedAt, usercreditrecord.FieldDeletedAt, usercreditrecord.FieldCreditsChange:
		...
		default:
			return nil, fmt.Errorf("unexpected column %q for type UserCreditRecord", columns[i])
		}
	}
	return values, nil
}
...
func (ucr *UserCreditRecord) assignValues(columns []string, values []interface{}) error {
...
func (ucr *UserCreditRecord) Update() *UserCreditRecordUpdateOne {
	return (&UserCreditRecordClient{config: ucr.config}).UpdateOne(ucr)
}
...
func (ucr *UserCreditRecord) Unwrap() *UserCreditRecord {
	...
	if !ok {
		panic("ent: UserCreditRecord is not a transactional entity")
	}
...
func (ucr UserCreditRecords) config(cfg config) {
	for _i := range ucr {
		ucr[_i].config = cfg
	}
}
